# deSitter Rebuild Plan — From the Ground Up
...
deSitter is an **audit scaffold for empirical research** — a structured graph
...
AI agents are the primary target user. deSitter ships an MCP server so that
...
deSitter is a **control plane over a research data plane**. The epistemic web
...
- `_sync_compatibility_state()` in `desitter.py` is ~120 lines of
...
| Consumer model for analyses | Correctness — deSitter records results, does not execute code |
...
modeled deSitter mostly as a domain library with a CLI on top. That is too
small. deSitter is a **control plane** whose domain kernel is the epistemic
web.
...
should be `src/desitter/`. `epistemic/` belongs *inside* that package
...
5. **Results ingestion** — the researcher runs analyses using their own tools
     and records results via `ds record` or the `record_result` MCP tool.
     deSitter is a consumer, not an executor.
...
- **Single config module**: `config.py` is the only place that reads
    `desitter.toml`. All services receive a `ProjectContext` — never the raw file.

### 3.4 Package Layout

All code paths below are relative to `src/desitter/` unless noted
otherwise. The Python import root is `desitter`.

```
src/
└── desitter/               # Top-level product package
    ├── __init__.py
    ├── __main__.py                 # `python -m desitter`
...
`ds show` renders `doi:` and `arxiv:` sources as clickable links.
...
The most important constraint on deSitter's design is what it does **not** do.
...
deSitter's job is to make auditing *possible*: ensuring every link in the chain has a
...
Every output deSitter produces is a structural observation about the state of the graph —
...
Rather than doing the reasoning, deSitter exposes navigation primitives that enable reasoning:
...
domain knowledge deSitter does not have. A human researcher sees the same data in readable form.
...
| **Analysis** | Researcher-run analytical work; deSitter records its results | claims_covered → Claims (bidirectional with claim.analyses); uses_parameters → Parameters (bidirectional with parameter.used_in_analyses) |
...
deSitter's vocabulary is chosen to be **domain-neutral without sacrificing
...
| deSitter Term | Why It's General |
|-------------|-----------------|
...
Different fields use different vocabulary for the same concepts. deSitter
uses general terms; researchers should map their domain language:

| deSitter | Physics | Medicine | Machine Learning | Social Science |
|---------|---------|----------|-----------------|----------------|
...
class Analysis:
    ...
    deSitter does not run analyses. 'path' and 'command' are provenance
    pointers — the researcher runs the analysis in their own environment
    and records the result via `ds record`. The git SHA captured at
...
- **DRY**: Bidirectional link maintenance happens in exactly one place per
  relationship. The current deSitter system maintains these in gateway
...
deSitter system uses. The package is installable. Tests cover round-trips.
...
deSitter system for backward compatibility.
"""
from __future__ import annotations
...
[project.scripts]
ds = "desitter.interfaces.cli.main:main"
...
`ds` command benefits from better argument parsing and beautiful output.
Compute libraries are opt-in for analysiss that need them.

Use `desitter` as the public and internal package root. The stable
contract is the `desitter` import root plus the `ds` console
...
from desitter.interfaces.cli.main import main
...
This keeps `python -m desitter` and the console-script entrypoint
...
- `desitter.__main__` supports `python -m desitter`
...
`desitter` is a normal importable package, not just a CLI tool.
Programmers who want to integrate deSitter into notebooks, pipelines, or
custom agents should be able to do so without going through the CLI or MCP:

```python
from desitter.controlplane.context import build_project_context
...
The `desitter/__init__.py` should re-export the most common entry
points so users don't have to memorize subpackage paths:

```python
# desitter/__init__.py
...
| 2.4 | `pyproject.toml` + `src/desitter/` package — installable product boundary | `pip install -e .`, console-script smoke, `python -m desitter` |
...
- [ ] Public console scripts and `python -m desitter` are stable
...
**Goal:** Build the product layer that makes deSitter a control plane again:
...
- `ds log claim C-003` (show mutation history per resource)
...
- [ ] Markdown output matches current deSitter behavior for the same data
...
def build_server(context: ProjectContext) -> MCPServer:
    ...
    server = MCPServer(name="desitter")
...
[project.scripts]
ds = "desitter.interfaces.cli.main:main"
ds-mcp = "desitter.interfaces.mcp.server:main"
```
...
def load_config(workspace: Path) -> DesitterConfig:
    """Load from desitter.toml if present, else use defaults."""
    toml_path = workspace / "desitter.toml"
    if toml_path.exists():
        raw = tomllib.loads(toml_path.read_text())
        return DesitterConfig(
            project_dir=Path(raw.get("desitter", {}).get("project_dir", "project")),
        )
    return DesitterConfig()
```
...
`ds init` is the platform onboarding experience, not just a scaffolding
...
✓ Created desitter.toml

Run `ds health` or connect your AI agent via `ds-mcp`.
```
...
desitter.toml                ← tool config (project_dir)
```

**`project/data/project_config.json`:**

Project metadata separate from tool configuration (`desitter.toml`).
Holds the project name, description, and any researcher-level annotations.
Mutable via `ds config set/get` — not a file users need to hand-edit.
...
| Step | What | Tests |
|------|------|-------|
| 4.1 | `config.py` — load from desitter.toml with defaults | Config with/without toml, custom project_dir |
...
| 4.4 | `ds-mcp` console-script entry point registered and runnable | Smoke test |
...
| 4.7 | `ds init` | Creates correct directory structure; idempotent |
| 4.8 | `ds health` (CLI + MCP tool) | Reports known issues in test fixture |
| 4.9 | `ds export --format=json` | Exports all data as valid JSON |
...
ds init
ds register claim C-001 '{"statement":"...", "type":"P", ...}'
ds validate --json
ds health
```
...
- [ ] `ds-mcp` console script entry point is registered and tested
- [ ] CLI dispatches all core commands
- [ ] `ds init` creates valid project scaffold (idempotent)
- [ ] `ds health` reports health accurately (CLI + MCP)
- [ ] Both human and JSON CLI output modes work
- [ ] Config is optional (defaults work without desitter.toml)
...
| # | Feature | Description |
|---|---------|-------------|
| 1 | Pretty-print output | `ds list claims` shows a Rich table. `--json` for machine output. |
| 2 | `ds add <type>` | Interactive prompts. Asks required fields one at a time. No JSON. |
| 3 | `ds show <type> [id]` | Human-readable view with all relationships rendered. `source` shown as clickable link. |
| 4 | `ds status` | Readable summary with health counts and tier-A evidence summary. |
| 5 | `ds log [id]` | Mutation history from the transaction log. |
| 6 | Shell completions | Generated for bash, zsh, and fish. Resource ID tab-completion from local data files. |
| 7 | `ds config set\|get` | Read/write `project_config.json` without editing JSON directly. |
...
For `ds add prediction`, also prompt for:
...
- No `ds dashboard`. `status` is enough.
- No static HTML rendering. Markdown is fine for now.
- `ds inspect` reports structural facts about the web. It does not
...
- [ ] `ds add prediction` collects specification and derivation prose
- [ ] `ds show` displays entity with relationships; `source` rendered as link
- [ ] `ds log` shows per-resource mutation history from provenance data
- [ ] Shell completions generated for bash, zsh, and fish
- [ ] `ds config set/get` reads and writes `project_config.json` safely
...
**Goal:** deSitter consumes results from analyses the researcher runs in
their own environment. No sandboxing, no execution, no subprocess management.
deSitter records what happened, links it to the epistemic web, and updates
prediction status.

This is the MLflow/W&B model: you instrument your work with a line or two,
and deSitter records the outcome. The computation lives in your tools —
SageMath, Python, R, Jupyter, whatever you trust. deSitter stays focused
on the epistemic layer.

### 10.1 Why Consumer, Not Executor

Running code introduces problems orthogonal to deSitter's core value:
...
- **Trust**: if deSitter runs the analysis, it owns the result. If the researcher
  runs it, they own the result — which is the correct epistemic relationship

The analyses deSitter tracks are not deSitter's analyses. They belong to the
researcher. deSitter records what they found.
...
**CLI:**
```bash
ds record P-007 --value 80.379 --status pass --notes "within 0.1% tolerance"
ds record P-007 --status fail --notes "derivation has sign error in step 3"
```
...
class AnalysisResult:
    ...
    git_sha: str | None = None           # auto-captured by `ds record` if workspace is a git repo
...
| 6.3 | `ds record <prediction_id>` CLI command | `--value`, `--uncertainty`, `--status`, `--notes`, `--no-transition`, `--json` flags work |
| 6.4 | `record_result` MCP tool | Returns correct `GatewayResult` envelope |
| 6.5 | `desitter.record()` SDK shim | Callable from any Python script; delegates to gateway |
| 6.6 | `controlplane/export.py` — `export_json`, `export_markdown` | Exports include recorded results alongside predictions |
| 6.7 | `ds results <prediction_id>` — show result history | Loads from `data/results.json`; shows value, uncertainty, status, git_sha, timestamp |
| 6.8 | Git SHA auto-capture in `ds record` | Captured from `git rev-parse HEAD`; warns if analysis `path` has uncommitted changes |
...
- [ ] Result history is visible via `ds results <id>`
- [ ] Export includes recorded results
- [ ] `ds record` warns when analysis file has uncommitted changes at record time
...
- Schema versioning + `ds migrate`
- `ds export --format=csv|notebook|bibtex`
- `ds import` from BibTeX/CSV
- Research graph traversal — `ds trace`, `ds impact`
- Example projects (`examples/` shipped with the package)
- `ds resolve-conflicts` — JSON-structure-aware merge for multi-user scenarios
- Static HTML rendering — `ds render --format=html`
- `ds health` as a CI exit code
...
|-----------|-------------------|-------|
| **Reuse/Release Equivalence (REP)** | The public release boundary is `desitter`; the kernel lives in `desitter.epistemic`, and internal modules can change behind that stable root without changing console scripts | Phase 2 packaging |
...
| **Convention over Configuration** | Default `project/` layout and minimal `desitter.toml`; declarative automation graph only for the places that truly vary | `config.py`, `controlplane/automation.py` |
...
**Nothing** from `src/horizon_core/` or `src/desitter.py` is imported,
subclassed, or wrapped. The new `src/desitter/` package is a
standalone implementation that happens to read the same JSON format.

### 14.3 Transition Plan

1. Build `src/desitter/` as a new package tree
...
- deSitter is rebuilt as a **control plane** over a project **data plane**.
...
- Git integration is for results ingestion only: `ds record` captures the
  git SHA at record time. Core operations never require git. deSitter works for
  researchers using Dropbox, OneDrive, or a plain local folder.
- Distribution name: `desitter`.
- Import namespace: `epistemic` (internal), `desitter` (public).
- Primary CLI: `ds`.
- Optional CLI alias: `desitter`.
...
Module imports: `src/desitter/interfaces/cli/main.py` →
`src/desitter/controlplane/validate.py` →
`src/desitter/epistemic/invariants.py`,
`src/desitter/adapters/json_repository.py`. No cycles.
...
### 17.2 Trace: `ds register claim C-099 '{...}'`
...
### 17.3 Trace: `ds run-script SCR-001 --json`
...
| --- | --- | --- |
| Domain model unit tests | In-memory only | `src/desitter/epistemic/web.py`, `src/desitter/epistemic/invariants.py` |
| Repository round-trip tests | Product fixture | `src/desitter/adapters/json_repository.py` |
| Validator unit tests | Product fixture | `src/desitter/controlplane/validate.py` |
| Renderer snapshot tests | Product fixture | `src/desitter/views/render.py` |
| Automation/config contract tests | Parse files as data | `src/desitter/controlplane/automation.py`, `pyproject.toml`, workflow files |
| Gateway integration tests | Product fixture + InMemoryRepo | `src/desitter/controlplane/gateway.py` |
| Rollback tests | Product fixture | `src/desitter/controlplane/gateway.py` |
| CLI integration tests | Repo fixture or subprocess | `src/desitter/interfaces/cli/main.py` |
| Config loading tests | Temp dirs | `src/desitter/config.py` |
...
No test ever needs to run `_sync_compatibility_state()` or import `desitter.py`
...
Is it parsing sys.argv or formatting CLI output?
    → src/desitter/interfaces/cli/main.py, interfaces/cli/formatters.py

Is it an MCP tool handler or FastMCP server wiring?
    → src/desitter/interfaces/mcp/server.py, interfaces/mcp/tools.py
...
Is it validating data for correctness?
    → src/desitter/controlplane/validate.py (orchestration) or src/desitter/epistemic/invariants.py (pure rules)

Is it generating markdown from JSON?
    → src/desitter/views/render.py (incremental) or src/desitter/adapters/markdown_renderer.py (templates)
...
Is it displaying project state or health summaries?
    → src/desitter/views/status.py or src/desitter/views/health.py

Is it recording analysis results from external tools?
    → src/desitter/controlplane/results.py (logic) or src/desitter/adapters/results_repository.py (persistence)
...
Is it pure domain logic (entities, invariants, graph queries)?
    → src/desitter/epistemic/
```

**The six rules:**

1. If it parses `sys.argv`, it goes in `src/desitter/interfaces/cli/`.
2. If it is an MCP tool handler, it goes in `src/desitter/interfaces/mcp/`.
3. If it computes paths from config, it goes in `src/desitter/config.py`.
4. If it expresses source → output or stale-trigger wiring, it goes in `src/desitter/controlplane/automation.py`.
5. If it mutates canonical JSON, it goes through `src/desitter/controlplane/gateway.py`.
6. If it only reads data and returns findings, it goes in `src/desitter/controlplane/validate.py` or `src/desitter/epistemic/invariants.py`.
...
For deSitter, CI is not just a unit-test runner. It is an **off-machine audit
...
4. **By the end of Phase 4:** add end-to-end product CI that exercises the
    installable CLI (`ds init`, register, validate, render, health,
...
- `ds render --check --json`
- `ds check-refs --all --json`
- `ds status --json`
...
- smoke `ds version`, `python -m desitter version`, and a tiny
  `ds init` flow
...
- `desitter.__main__` remains functional
